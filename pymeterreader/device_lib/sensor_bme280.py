...
from pymeterreader.device_lib.common import Sample, Device, Channel
...
class Bme280Reader(BaseReader):
    ...
    def __init__(self, meter_id: str, **kwargs: int):
        ...
        try:
            ...
            error("Could not import smbus library! This library is missing and Bme280Reader can not function without it!")
            raise e
        if kwargs:
    ...
    def poll(self) -> tp.Optional[Sample]:
        ...
        with self.__I2C_LOCK:
            try:
                ...
                sample.channels.append(Channel('TEMPERATURE', temperature / 100.0, '°C'))
                ...
                sample.channels.append(Channel('PRESSURE', pressure / 100.0, 'hPa'))
                ...
                if humidity > 100:
                    ...
                    humidity = 0
                sample.channels.append(Channel('HUMIDITY', humidity, '%'))
