...
class SmlSimulator(Thread):
    ...
    def __init__(self, sml_message: bytes) -> None:
        self.__continue = True
        self.__sml_message: bytes = sml_message
        super().__init__()
...
class TestSmlReader(unittest.TestCase):
    ...
    def test_init(self, serial_for_url_mock):
        ...
        simulator = SmlSimulator(SML_TEST_BYTES)
        simulator.start()
        reader = SmlReader("1EMH004921570", "loop://")
        sample = reader.poll()
        simulator.stop()
        self.assertEqual(sample.meter_id, SML_TEST_ID)
        self.assertEqual(sample.channels, SML_TEST_CHANNELS)
    ...
    def test_detect(self, serial_for_url_mock, list_ports_mock):
        ...
        # Add Sideeffect that starts the SmlSimulator
        ...
        self.simulator = SmlSimulator(SML_TEST_BYTES)
        # Start device detection
        devices = SmlReader("irrelevent", "unused://").detect()
        self.simulator.stop()
        self.assertEqual(len(devices), 1)
        self.assertIn(Device('1 EMH 00 4921570', '/dev/ttyUSB1', 'SML', SML_TEST_CHANNELS), devices)
